...
(up, down, left, right, or diagonally) in the 20×20 grid written in the code below? 
*/ 
...
namespace ConsoleApplication59
{
    class Program
    {
        ...
        {
            int[,] num_matrix = {{8,2,22,97,38,15,0,4,0,75,4,5,7,78,52,12,5,77,91,8}
            ...
            for (int i = 0; i < 20; i++)
            {
                for (int r = 0; r < 16; r++) //Check the rows 
                {
                    prod = num_matrix[i, r] * num_matrix[i, r + 1] * num_matrix[i, r + 2] * num_matrix[i, r + 3];
                    if (prod > max_prod)
                        max_prod = prod;
                }

                for (int c = 0; c < 16; c++) //Checks the columns
                {
                    prod = num_matrix[c, i] * num_matrix[c + 1, i] * num_matrix[c + 2, i] * num_matrix[c + 3, i];
                    if (prod > max_prod)
                        max_prod = prod;
                }

                if (i < 16) //Checks the right diagonal 
                {
                    for (int diag = 0; diag < 16; diag++)
                    {
                        prod = num_matrix[i, diag] * num_matrix[i + 1, diag + 1] *
                               num_matrix[i + 2, diag + 2] * num_matrix[i + 3, diag + 3];
                        if (prod > max_prod)
                            max_prod = prod;
                    }


                    for (int l_diag = 19; l_diag > 4; l_diag--) //Checks the left diagonal 
                    {
                        prod = num_matrix[l_diag, i] * num_matrix[l_diag - 1, i + 1] *
                               num_matrix[l_diag - 2, i + 2] * num_matrix[l_diag - 3, i + 3];
                        if (prod > max_prod)
                            max_prod = prod;
                    }
